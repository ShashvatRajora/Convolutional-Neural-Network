...
int main(int argc, char* argv[]) {
    ...
    // 🔁 Check for "--load" flag and model file
    if (argc > 1 && strcmp(argv[1], "--load") == 0) {
        FILE* f = fopen("logs/trained_model.txt", "r");
        if (!f) {
            printf("❌ trained_model.txt not found. Please train the model first.\n");
            return 1;
        }
        fclose(f);
        load_model(&conv1, &conv2, &fc, "logs/trained_model.txt");
        printf("✅ Model loaded from file. Skipping training...\n");
    } else {
        // 🚀 Training Loop
